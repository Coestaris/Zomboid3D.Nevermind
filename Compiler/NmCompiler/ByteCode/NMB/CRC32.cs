...
namespace Nevermind.ByteCode.NMB
{
    internal class CRC32
    {
        ...
        private static UInt32 UpdateCRC32(byte ch, UInt32 crc)
        {
            ...
            UInt32 tabValue = CRC32Tab[idx];
            return tabValue ^ ((crc) >> 8);
        }

        private static unsafe UInt32 _CRC32(byte* data, UInt32 dataLen)
        {
        ...
        public static unsafe UInt32 ComputeChecksum(byte[] bytes)
        {
            fixed (byte* _bytes = bytes)
                return _CRC32(_bytes, (UInt32)bytes.Length);
        }
    }
}